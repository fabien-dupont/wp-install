...
WPLAB_DIR=$(echo '/Applications/MAMP/htdocs')
PROJECT_NAME=''
...
wp_setup() {
  ...
  /Applications/MAMP/Library/bin/./mysql --host=localhost -uroot -p -e "CREATE DATABASE $PROJECT_NAME" --default-character-set=utf8 ;
  wp core config --dbname=$PROJECT_NAME --dbuser=$DB_USER --dbpass=$DB_PASS --dbprefix=wp_ --skip-check ;  # we need to make it work without this : --skip-check  
};
...
wp_install() {
  ...
  echo '' ;
  wp core install --path=$PROJECT_DIR --url=$SITE_URL --title="$PROJECT_NAME" --admin_user=admin --admin_password=admin --admin_email=fabien@studio60.ch ;
...
theme_screenshot() {
  ...
  curl --silent https://raw.githubusercontent.com/fabien-dupont/wp-install/master/screenshot.png -O screenshot.png ;
...
wp_config() {
  ...
  echo ''
  wp theme delete  twentynineteen twentytwenty twentytwentyone twentytwentytwo twentytwentythree --path=$PROJECT_DIR
...
gulp_install() {
  ...
  curl --silent https://raw.githubusercontent.com/fabien-dupont/wp-install/main/config/wpgulp.config.js -O wpgulp.config.js ;
  #mv screenshot.png wp-content/themes/theme_studio_soixante/;
  #echo "const projectURL = 'localhost';" 		>		wpgulp_proxy.txt   &&
...
install_project() {
  ...
  SITE_URL=$(echo $LOCALHOST_APACHE_ADDRESS/$PROJECT_NAME/)
